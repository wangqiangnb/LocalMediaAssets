...
public sealed class ActorController : ControllerBase
{
    ...
    public ActionResult<ActorStatusResult> GetActorStatus([FromQuery] string itemId, [FromQuery] string? userId)
    {
        ...
        foreach (var p in people.Where(p => !string.IsNullOrWhiteSpace(p.Name)))
        {
            ...
            string? avatarUrl = hasAvatar
                ? "/LocalMediaAssets/Actor/Image?name=" + Uri.EscapeDataString(p.Name)
                : null;
        ...
        try
        {
            ...
            foreach (var item in items)
            {
                ...
                if (!string.IsNullOrWhiteSpace(excludeItemId)
                    && Guid.TryParse(excludeItemId, out var excludeGuid)
                    && item.Id == excludeGuid)
                {
                    continue;
                }

                list.Add(new AppearanceItem
                {
                    Id = item.Id.ToString(),
                    Name = item.Name,
