...
public sealed class ActorIndex
{
    ...
    public IReadOnlyList<ActorPhotoEntry> FindPhotos(string name, PluginConfiguration config)
    {
        ...
        lock (_sync)
        {
            ...
            // 过滤文件已删的（Jellyfin 源 URL 除外）
            var valid = list
                .Where(p => p.Source == ActorSource.Jellyfin || !string.IsNullOrEmpty(p.Path) && File.Exists(p.Path))
                .ToList();
            if (valid.Count == 0)
            {
                return [];
    ...
    /// <summary>
    /// 返回默认照片（严格同名优先；无则按文件大小最大者；Jellyfin 源作兜底）。
    /// </summary>
    public ActorPhotoEntry? FindDefaultPhoto(string name, PluginConfiguration config)
    {
        ...
        if (exact.Count > 0)
        {
            // 排除备用库/仅 Jellyfin 的严格同名作为默认（默认图优先来自可写来源）
            var writable = exact.Where(p => p.Source is ActorSource.Library or ActorSource.Video).ToList();
            return (writable.Count > 0 ? writable : exact).First();
        }

        // 无严格同名：取文件最大者（排除备用库，第三方图不抢默认）
        var candidates = photos
            .Where(p => p.Source is ActorSource.Library or ActorSource.Video)
            .OrderByDescending(p => p.SizeBytes)
            .ToList();
        return candidates.Count > 0 ? candidates[0] : photos[0];
    }
        ...
        Dictionary<string, DateTime> mtimes)
    {
        ...
        IndexFolderFiles(staging, ActorSource.Staging, index, infoFiles, includeSubLevel: false);
        TryRecordMtime(mtimes, staging);

        var peopleDir = Path.Combine(staging, _peopleFolderName);
        if (Directory.Exists(peopleDir))
        {
            IndexFolderFiles(peopleDir, ActorSource.Staging, index, infoFiles, includeSubLevel: true);
            TryRecordMtime(mtimes, peopleDir);
        }
    }
        ...
        bool includeSubLevel)
    {
        ...
        void AddFilesFrom(string dir)
        {
            ...
            foreach (var file in files)
            {
                ...
                var isInfo = InfoExtensions.Contains(ext, StringComparer.OrdinalIgnoreCase);
                if (!isImage && !isInfo)
                {
                    continue;
                }
